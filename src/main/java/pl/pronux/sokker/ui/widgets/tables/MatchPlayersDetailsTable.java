...
public class MatchPlayersDetailsTable extends SVTable<PlayerStats> implements IViewSort<PlayerStats> {
	...
	public MatchPlayersDetailsTable(Composite parent, int style) {
		...
		String[] columnsTooltips = { Messages.getString("table.match.substitutions.tooltip"), Messages.getString("table.match.number.tooltip"), Messages.getString("table.match.player.tooltip"),   
				Messages.getString("table.match.formation.tooltip"), Messages.getString("table.match.time.tooltip"), Messages.getString("table.match.rating.tooltip"),   
				Messages.getString("table.match.goals.tooltip"), Messages.getString("table.match.shoots.tooltip"), Messages.getString("table.match.assists.tooltip"),   
				Messages.getString("table.match.fouls.tooltip"), Messages.getString("table.match.injury.tooltip"), Messages.getString("table.match.cards.tooltip"), " " };    

		String[] columns = { " ", Messages.getString("table.match.number"), Messages.getString("table.match.player"), Messages.getString("table.match.formation"),    
				Messages.getString("table.match.time"), Messages.getString("table.match.rating"), Messages.getString("table.match.goals"), Messages.getString("table.match.shoots"),    
				Messages.getString("table.match.assists"), Messages.getString("table.match.fouls"), Messages.getString("table.match.injury"), Messages.getString("table.match.cards"), " " };     
				...
				if (player.getExistsInSokker() == Player.EXISTS_IN_SOKKER_TRUE || player.getExistsInSokker() == Player.EXISTS_IN_SOKKER_COMPLETED) {
					...
					if (player.getName() == null || player.getSurname() == null || (player.getName().isEmpty() && player.getSurname().isEmpty())) {
						...
						if(player.getName().isEmpty()) {
							...
							item.setText(i++, String.format("%s %s.", player.getSurname(), player.getName().substring(0, 1))); 
						}
					}
				} else {
