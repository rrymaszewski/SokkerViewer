...
public class BugReportAction {
	...
	public String sendBug(String bug, SokkerViewerSettings settings) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, ShortBufferException, IOException, BadArgumentException {
		...
		parameters = Crypto.encodeBase64(Crypto.encryptSymmetric((SettingsHandler.getSokkerViewerSettings().getUsername() + ";;" + bug + ";;" + getEnvironment()).getBytes("UTF-8"), skey, SYMMETRIC_KEY_SEQUENCE)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
